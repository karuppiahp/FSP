...
public class SettingsCollegeEditTask extends AsyncTask<Void, Void, Boolean> {
	ProgressDialog dialog;
	Activity activity;
	String status, message, roleId, schoolId;
	JSONObject jsonObj;

	public SettingsCollegeEditTask(Activity context, String roleid, String schoolid) {
		activity = context;
		roleId = roleid;
		schoolId = schoolid;
	...
	protected Boolean doInBackground(Void... params) {
		...
		nameValuePairs.add(new BasicNameValuePair("id", SessionStores.getUserId(activity)));
		nameValuePairs.add(new BasicNameValuePair("role_id", roleId));
		nameValuePairs.add(new BasicNameValuePair("school_id", schoolId));
		jsonObj = new ServerResponse(UrlGenerator.settingsCollegeEdit())
