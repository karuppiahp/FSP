...
public class RegisterTask {
    ...
    public RegisterTask(Context context, ApiCallParams apiCallParams, FragmentManager fragmentManager) {
        ...
        new ServerResponse(apiCallParams.getUrl()).getJSONObjectfromURL(ServerResponse.RequestType.POST, apiCallParams.getParams(), context, new VolleyResponseListener() {
            ...
            public void onResponse(String response) { //Success response
                ...
                try {
                    ...
                    if (jObject != null) {
                        if (jObject.has("access_token")) { //if access token available
                            new SessionStores(context).saveAccessToken("" + jObject.getString("access_token"));
                            new SessionStores(context).saveUserEmail("" + SessionStores.USER_PROFILE.getEmail());
                            new SessionStores(context).saveUserPhone("" + SessionStores.USER_PROFILE.getPhone());
                            ((FragmentMain) context).setLoginRegText("LOG IN");
                            //Fragment replaces with OTP screen
                            FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
                            OtpScreen fragment = new OtpScreen();
                            Bundle args = new Bundle();
                            args.putString("from", "register");
