...
public class HandicapAccessTask extends AsyncTask<Void, Void, Boolean> {
	ProgressDialog dialog;
	Activity activity;
	String status;
	JSONObject jsonObj;
	
	public HandicapAccessTask(Activity context, String deviceStatus) {
		activity = context;
		status = deviceStatus;
	...
	protected Boolean doInBackground(Void... params) {
		...
		nameValuePairs.add(new BasicNameValuePair("user_role", SessionStores.getRoleName(activity)));
		nameValuePairs.add(new BasicNameValuePair("status", status));
		jsonObj = new ServerResponse(UrlGenerator.handicapAccess())
